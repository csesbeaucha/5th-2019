...
import team10, team11, team12, team13, team14,team15,team16,team17,team18
betray = example1
collude = example0
...
def play_round(player1, player2, score1, score2, moves1, moves2):
    ...
    RELEASE = 50 # (R, "reward" in literature) when both players collude
...
def make_section1(modules, scores):
    ...
    for index in range(len(modules)):
        ...
        for i in range(len(modules)):
            section1 += '{:>7}'.format(round(scores[i][index]),2)
        section1 += '\n'
...
def make_section2(modules, scores):
    ...
    section2_list.sort(key=lambda x: x[2] , reverse=False)  #int(x[2])
...
def make_section3(modules, moves, scores, index):
    ...
    ----------------------------------------------------------------------------
    Section 3 - Game Data for Team Colloid c=-500 b=-250 C=+50 B=+100
    ----------------------------------------------------------------------------
